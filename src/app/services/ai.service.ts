...
export class AiService {
   ...
   * @private
   */
  private async callClaudeApi(pdfText: string, filename: string): Promise<{summary: string[]}> {
    try {
      // Apply rate limiting before making the API call
      await this.rateLimiter.checkRateLimit('anthropic');
      ...
      // Dynamically import the GenKit flow to avoid SSR issues
      const { summaryFlow } = await import('../../genkit/summary.flow');
      
      // Call the GenKit flow with PDF text and filename
      // In the new GenKit structure, the flow is directly callable
      const result = await summaryFlow({
        pdfText,
        filename
      });
      ...
      // Register successful request with rate limiter
      this.rateLimiter.registerRequest('anthropic', true);
      ...
      // Register failed request with rate limiter
      this.rateLimiter.registerRequest('anthropic', false);
